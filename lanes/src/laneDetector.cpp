...
bool checkPrimaryLane(Mat boundary, Mat edges, Mat intensityMaxima)
{
    ...
    // maiking a feature image similar to size of edges and initilalizing it with 0
...
vector<Point> fitCurve(Mat img, Mat boundary, Mat edges, Mat intensityMaxima, bool debug=false)
{
    ...
    for(int i=0; i<lanes.rows ; i++)
        for(int j=0; j<lanes.cols ; j++)
    {
        if(lanes.at<Vec3b>(i,j)[0] == 255 && lanes.at<Vec3b>(i,j)[1] == 0 && lanes.at<Vec3b>(i,j)[2] == 0 && (i == 260 || i == 350))
        {
            //cout<<i<<"\t"<<j+3<<endl;
            temp.push_back(Point(i,j+3));
            j+=20;
        }
    }
...
public:
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/camera/image_color", 1,&ImageConverter::imageCb, this);
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    lanepub = nh_.advertise<geometry_msgs::Polygon>("lane_data", 100);
