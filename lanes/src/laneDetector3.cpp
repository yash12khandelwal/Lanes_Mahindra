...
int evaluate(Mat img,bool debug,int frame){
	...
	label_name = string(6-int_to_string(frame).length(),'0')+int_to_string(frame)+".png";
	Mat label_img = imread("/home/tejus/Documents/vision/lane-detector/labeller/sequence_1/"+label_name);
	...
	if(num_points>=1 && frame>=812 && tracking_status)
...
Mat findIntensityMaxima(Mat img, bool debug)
{
    ...
    for(int i=0;i<t.rows;i++)
        ...
        {
            //t.at<float>(i, j) = t0.at<float>(i, j);
...
void detect_lanes(Mat img, bool debug = true)
{
    ...
    start  = clock();
    Mat intensityMaxima = findIntensityMaxima(img, false);
    stop = clock();
    time1 += (stop-start)/double(CLOCKS_PER_SEC);
    start = clock();
    Mat edgeFeature = findEdgeFeatures(img, false, false);
    stop = clock();
    time2 += (stop-start)/double(CLOCKS_PER_SEC);

    start = clock();
    fitCurve(img, boundary, edgeFeature, intensityMaxima, false);
    stop = clock();
    time3 += (stop-start)/double(CLOCKS_PER_SEC);

    f++;

    cout<<"AVG TIME = "<<time1/f<<" "<<time2/f<<" "<<time3/f<<endl;
    cout<<curve<<endl;
}
...
int main()
{
    ...
    // int total_frame=0;
    // Mat img = imread("/home/yash/AGV/lanedetector/images/000040.png");
    // detect_lanes(img);
 //    eval_file.open("error3.txt");
	// int total_frame=0;
 ...
 //    }
    ...
    out.open( "outputVideo3.avi", CV_FOURCC('D','I','V','X'), 10, Size ( 1242, 375), true );
